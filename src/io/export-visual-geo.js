...
import { resolveEndpoint, findVisualLayer } from '../state/visual.js';
import { APP_NAME, APP_VERSION } from '../core/version.js';
import { writeGeo, geoCoordinateSystem, GeoWriteError } from './write-geo.js';

export const GEO_INCLUDE_ALL = { points: true, lines: true, areas: true };
 ...
 * @returns {{ points:object[], lines:object[], counts:{points:number, lines:number, areas:number}, skipped:number }}
 */
export function collectVisualGeo(state, { layerIds = null, objectIds = null, include = GEO_INCLUDE_ALL } = {}) {
  ...
  return {
    points, lines: [...lines, ...areas],
    counts: { points: points.length, lines: lines.length, areas: areas.length },
    skipped,
  };
}
